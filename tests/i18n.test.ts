...
describe('i18n dictionary', () => {
  ...
  it('keeps technical tokens and user data independent of language', () => { expect('Jasmine Study').toBe('Jasmine Study'); expect(messages.ko.parts).toBe('파트'); expect(messages.ko.date).toBe('날짜'); expect(messages.ko.actions).toBe('동작'); expect(messages.ko.percent).toBe('백분율'); expect(messages.ko.cas).toBe('CAS / 참조'); expect('SOLVENT').toBe('SOLVENT'); expect('DIL').toBe('DIL') })
