...
describe('Formula Drop routes', () => {
  ...
  it.each(['/operator/formula-storage', '/operator/formula-storage/'])('resolves the operator vault route', path => expect(resolveAccordbookRoute(path)).toEqual({ kind: 'formula-storage' }))
