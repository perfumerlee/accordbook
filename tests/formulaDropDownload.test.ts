...
const handoffCss = readFileSync(new URL('../src/components/dropDirectHandoff.css', import.meta.url), 'utf8')
describe('Formula Drop Phase 4 download contract', () => {
  ...
  it('reveals access values only after a successful active download', () => { expect(page).toContain('DOWNLOAD .ACCORDBOOK FILE'); expect(page).toContain('PREPARING…'); expect(page).toContain('FORMULA DOWNLOAD STARTED'); expect(page).toContain('accessPin') })
  it('keeps every Formula Drop CTA label readable across link states', () => {
    expect(page).toContain('OPEN IN ACCORDBOOK')
    expect(page).toContain('Accordbook에서 열기')
    expect(page).toContain('.accordbook 파일 다운로드')
    expect(handoffCss).toContain('.formula-drop-button:link')
    expect(handoffCss).toContain('.formula-drop-button:visited')
    expect(handoffCss).toContain('.formula-drop-button:hover')
    expect(handoffCss).toContain('.formula-drop-button:focus-visible')
    expect(handoffCss).toContain('.formula-drop-button:active')
    expect(handoffCss).toContain('color: var(--paper) !important')
  })
