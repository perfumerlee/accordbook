...
describe('Formula Drop Phase 4 download contract', () => {
  ...
  it('creates one new event ID per download and starts a browser download', () => { expect(download).toContain('createFormulaDropEventId()'); expect(download).toContain('link.download = result.download.fileName'); expect(download).toContain('link.click()') })
