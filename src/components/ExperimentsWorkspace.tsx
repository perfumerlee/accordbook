...
function NotebookHead({language}:{language:'en'|'ko'}){const resolved=experimentMessages[language];return <thead><tr><th>{resolved.parts}</th><th>{resolved.materialName}</th><th>{resolved.dilution}</th><th>{resolved.memo}</th><th>{resolved.deleteLabel}</th></tr></thead>}
function NotebookSummary({rows,note,language,onNote}:{rows:FormulaSnapshotRow[];note:string;language:'en'|'ko';onNote?:(value:string)=>void}){
 const ko=language==='ko'
...
function BaseNotebook({experiment,sourceLabel,language}:{experiment:Experiment;sourceLabel:string;language:'en'|'ko'}){
 const t=experimentMessages[language]
 return <section className="experiment-notebook"><div className="experiment-variant-heading"><span>BASE · {sourceLabel}</span><span className="experiment-read-only-badge">{t.readOnly}</span></div><table className="experiment-material-table"><NotebookHead language={language}/><tbody>{experiment.baseSnapshot.rows.map(row=><tr key={row.rowId}><td className="experiment-parts">{row.parts}</td><td className="experiment-material"><span>{row.material}</span></td><td className="experiment-dilution">{row.dilution?.enabled?<><button className="experiment-dilution-badge--active" type="button" aria-label="DIL" disabled>DIL</button><span className="experiment-dilution-value">@{row.dilution.percent}% in {row.dilution.solvent}</span></>:'—'}</td><td className="experiment-row-memo">{row.memo||'—'}</td><td className="experiment-delete-cell">—</td></tr>)}</tbody></table><NotebookSummary rows={experiment.baseSnapshot.rows} note={experiment.baseSnapshot.notes} language={language}/></section>
}
function VariantEditor({experiment,variant,onChange,onDelete,saving,language,structuralDisabled,evaluationDrafts,onEvaluationDraftsChange}:{experiment:Experiment;variant:import('../models/experiment').ExperimentVariant;onChange:(e:Experiment)=>void;onDelete:()=>void;saving:string;language:'en'|'ko';structuralDisabled:boolean;evaluationDrafts:EvaluationDrafts;onEvaluationDraftsChange:(drafts:EvaluationDrafts)=>void}){
 ...
 const saveLabel=saving.startsWith('SAVING')?(uiLanguage==='ko'?'저장 중…':'SAVING…'):saving==='SAVE FAILED'?(uiLanguage==='ko'?'저장 실패':'SAVE FAILED'):(uiLanguage==='ko'?'저장됨':'SAVED')
 const deleteVariantLabel=variant.parentVariantId?(uiLanguage==='ko'?'브랜치 삭제':'DELETE BRANCH'):(uiLanguage==='ko'?'시안 삭제':'DELETE VARIANT')
 ...
 return <section className="experiment-notebook">
  <div className="experiment-variant-heading"><div className="experiment-variant-title-group"><span>{uiLanguage==='ko'?'시안':'VARIANT'} {variant.label}</span><span className="experiment-save-status" data-state={saving.startsWith('SAVING')?'saving':saving==='SAVE FAILED'?'failed':'saved'} role="status" aria-live="polite">{saveLabel}</span></div><div className="experiment-variant-heading__actions"><>{canBranch&&!structuralDisabled&&<button className="experiment-create-branch" type="button" aria-label={branchMessages[uiLanguage].createBranchFrom(variant.label)} onClick={createBranch}>{branchMessages[uiLanguage].addBranch}</button>}<div className="experiment-delete-group"><button className="experiment-delete-variant" type="button" disabled={deleteBlocked||structuralDisabled} aria-describedby={deleteBlocked?'experiment-delete-blocked':undefined} onClick={onDelete}>{deleteVariantLabel}</button>{deleteBlocked&&<p id="experiment-delete-blocked" className="experiment-delete-blocked">{branchMessages[language].deleteBlocked}</p>}</div></></div></div>
  {branchIntentOpen&&<form className="branch-intent-form" onSubmit={submitBranch}><BranchIntentFields value={branchIntent} onChange={setBranchIntent} language={language} purpose="development" disabled={structuralDisabled}/><div className="evaluation-actions"><button type="submit" disabled={structuralDisabled||!branchIntent.changeIntent.trim()||!branchIntent.hypothesis.trim()}>{language==='ko'?'브랜치 만들기':'Create Branch'}</button><button type="button" disabled={structuralDisabled} onClick={()=>setBranchIntentOpen(false)}>{language==='ko'?'취소':'Cancel'}</button></div></form>}
  <BranchRecord variant={variant} parentLabel={getVariantParent(experiment,variant.variantId)?.label} language={language}/><table className="experiment-material-table"><NotebookHead language={language}/><tbody>{variant.snapshot.rows.map(row=><tr className={isChangedFromBase(row)?'experiment-row--marked':''} key={row.rowId}>
   <td className="experiment-parts"><input aria-label={uiLanguage==='ko'?'배합량':'Parts'} type="number" value={row.parts} onChange={e=>update(row.rowId,{parts:e.target.value===''?'':Number(e.target.value)})}/></td>
   <td className="experiment-material"><input aria-label={uiLanguage==='ko'?'원료명':'Material name'} disabled={baseRowIds.has(row.rowId)} value={row.material} onChange={e=>update(row.rowId,{material:e.target.value})}/></td>
   <td><div className="experiment-dilution"><button type="button" aria-label={uiLanguage==='ko'?'희석':'Dilution'} disabled={baseRowIds.has(row.rowId)} aria-pressed={row.dilution?.enabled??false} onClick={()=>update(row.rowId,{dilution:{enabled:!row.dilution?.enabled,percent:row.dilution?.percent??1,solvent:row.dilution?.solvent??'ALC'}})}>DIL</button>{row.dilution?.enabled&&baseRowIds.has(row.rowId)&&<span className="experiment-dilution-value">{uiLanguage==='ko'?`${row.dilution.solvent} · ${row.dilution.percent}% 희석`:`@${row.dilution.percent}% in ${row.dilution.solvent}`}</span>}{row.dilution?.enabled&&!baseRowIds.has(row.rowId)&&<div className="experiment-dilution-fields"><span>@</span><input aria-label={uiLanguage==='ko'?'희석 비율':'Dilution percent'} type="number" value={row.dilution.percent} onChange={e=>update(row.rowId,{dilution:{...row.dilution!,percent:Number(e.target.value)}})}/><span>% {uiLanguage==='ko'?'희석 용매':'in'}</span><input aria-label={uiLanguage==='ko'?'희석 용매':'Dilution solvent'} value={row.dilution.solvent} onChange={e=>update(row.rowId,{dilution:{...row.dilution!,solvent:e.target.value}})}/></div>}</div></td>
   <td className="experiment-row-memo"><input aria-label={uiLanguage==='ko'?`${row.material||'원료'} 메모`:`Memo for ${row.material||'material'}`} placeholder={uiLanguage==='ko'?'메모':'MEMO'} value={row.memo??''} onChange={e=>update(row.rowId,{memo:e.target.value})}/></td>
   <td><button className="experiment-remove" type="button" aria-label={uiLanguage==='ko'?'원료 삭제':'Remove material'} onClick={()=>onChange(removeVariantRow(experiment,variant.variantId,row.rowId))}>×</button></td>
  </tr>)}</tbody></table>
  ...
  <NotebookSummary rows={variant.snapshot.rows} note={variant.note} language={language} onNote={value=>onChange(updateVariantNote(experiment,variant.variantId,value))}/>
