...
function formatSnapshotOrigin(snapshot: FormulaVersionSnapshot, language: 'en' | 'ko'): string {
  const source = snapshot.claimedSource
  if (!source || source.originType === 'not_specified') return '—'
  const t = messages[language]
  const labels = { original: t.originOriginal, inspired_by: t.originInspired, adapted_from: t.originAdapted, imported: language === 'ko' ? '가져옴' : 'Imported from', duplicated: language === 'ko' ? '복제됨' : 'Duplicated from', reference: language === 'ko' ? '참고' : 'Reference', unknown: language === 'ko' ? '출처 불확실' : 'Unknown origin' }
  const label = labels[source.originType]
  const detail = source.title || source.creator || source.author || source.note || source.sourceTitle || source.reference
...
export default function TimeMachinePanel({ formula, storage, language, onClose, onBeforeSaveVersion, onRestore, onCreateAsNew, onMarkReleasedVersion, onRemoveReleasedVersion, onViewAsSheet, isOpen, opener, openSequence = 0 }: { formula: Formula; storage: AccordbookStorage; language: 'en' | 'ko'; onClose: () => void; onBeforeSaveVersion?: () => Promise<void>; onRestore?: (formula: Formula) => Promise<void>; onCreateAsNew?: (version: FormulaVersion) => Promise<void>; onMarkReleasedVersion?: (formulaId: string, versionId: string) => Promise<void>; onRemoveReleasedVersion?: (formulaId: string) => Promise<void>; onViewAsSheet?: (versions: FormulaVersion[], includeCurrent: boolean) => void; isOpen: boolean; opener?: HTMLElement | null; openSequence?: number }) {
  ...
  const toggleSheetVersion = (versionId: string) => {
    setSheetSelected(current => {
      if (current.includes(versionId)) return current.filter(id => id !== versionId)
      if (current.length + (sheetCurrent ? 1 : 0) >= 5) return current
      return [...current, versionId]
    })
  }
  const toggleSheetCurrent = () => {
    setSheetCurrent(current => current ? false : sheetSelected.length < 5)
  }
  const handleViewAsSheet = () => {
    if (!onViewAsSheet || sheetSelectionCount < 2 || sheetSelectionCount > 5) return
    const selectedVersions = versions.filter(version => version.parentFormulaId === formula.id && sheetSelected.includes(version.versionId))
    if (selectedVersions.length !== sheetSelected.length) return
    onViewAsSheet(selectedVersions, sheetCurrent)
  }
  const releasedVersion = getReleasedVersion(formula, versions)
  ...
  const handleBackdropClick = (event: React.MouseEvent<HTMLDivElement>) => {
    if (companion || event.target !== event.currentTarget || capitalizationOpen || restoreConfirm) return
    requestClose()
  }
  ...
  return <div className={`tm-backdrop ${closing ? 'is-closing' : ''}`} onClick={handleBackdropClick}>
    <section ref={panelRef} tabIndex={-1} inert={!isOpen} onKeyDown={handlePanelKeyDown} className={'tm-panel ' + (selected ? 'tm-detail ' : '') + ((contextChanging || formulaContextChanged) ? 'tm-context-switch' : '')} role="dialog" aria-modal={!companion ? true : undefined} aria-label="Time Machine">
      ...
      <div className="tm-content" ref={contentRef}>
        <div id="tm-view-version" role="tabpanel" aria-labelledby="tm-tab-version" hidden={batchOpen || compositionOpen || activeTab !== 'version'}>
          {selected ? <><button className="tm-version-list-back" type="button" onClick={() => setSelected(undefined)}>← {t.batchVersionList}</button><h2 className="tm-version-title">{selected.kind === 'manual' ? `v${selected.versionNumber}` : (ko ? '복원 지점' : 'RESTORE POINT')}</h2><div className="tm-version-meta"><span>{ko ? '읽기 전용' : 'READ ONLY'}</span><time>{date(selected.createdAt)}</time></div><VersionReleaseStatus key={selected.versionId} formula={formula} version={selected} versions={versions} language={language} onConfirm={onMarkReleasedVersion} onRemove={onRemoveReleasedVersion} />{selected.note && <div className="tm-note"><span>{ko ? '버전 메모' : 'VERSION NOTE'}</span><p>{selected.note}</p></div>}{Array.isArray(selected.snapshot?.rows) ? <HistoricalFormula snapshot={selected.snapshot} language={language} /> : <p className="tm-batch-helper">{t.batchUnavailable}</p>}<div className={`tm-restore-action ${!restoreConfirm ? 'tm-version-actions-grid' : ''}`}>{restoreConfirm ? <div className="tm-restore-confirm" role="alertdialog" aria-label={ko ? '복원 확인' : 'Restore confirmation'}><strong>{ko ? `v${selected.versionNumber ?? ''}를 현재 포뮬러로 복원할까요?` : `Restore ${selected.kind === 'manual' ? `v${selected.versionNumber}` : 'this restore point'} to Current?`}</strong><p>{ko ? '현재 포뮬러의 내용이 이 버전으로 변경됩니다. 기존 버전 기록은 그대로 유지됩니다.' : 'The current formula will be replaced with this version. Your version history will be kept.'}</p><div><button className="btn" type="button" onClick={() => setRestoreConfirm(false)}>{ko ? '취소' : 'CANCEL'}</button><button className="btn primary" type="button" disabled={restoring} onClick={() => void restore()}>{restoring ? '…' : (ko ? '복원' : 'RESTORE')}</button></div></div> : <><button className="tm-restore-button" type="button" onClick={() => setRestoreConfirm(true)}><svg className="tm-restore-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M9 7 5 11l4 4" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/><path d="M6 11h7a6 6 0 0 1 6 6" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg><span>{ko ? '이 버전 복원' : 'RESTORE THIS VERSION'}</span></button><button className="tm-restore-button" type="button" disabled={creatingAsNew} onClick={() => void createAsNew()}>{creatingAsNew ? '…' : (ko ? '새 포뮬러로 만들기' : 'CREATE AS NEW')}</button></>}<button ref={batchActionRef} className="tm-restore-button tm-make-batch" type="button" onClick={openBatch}>{t.makeBatch}</button>{!restoreConfirm && canShowVersionComposition(selected) && <button ref={compositionActionRef} className="tm-restore-button tm-composition-action" type="button" onClick={openComposition}>{t.composition}</button>}</div><button className="tm-detail-back" type="button" onClick={() => setSelected(undefined)}>← TIME MACHINE</button></> : <div className="tm-timeline"><div className="tm-current"><label><input type="checkbox" checked={sheetCurrent} disabled={!sheetCurrent && sheetSelectionCount >= 5} onChange={toggleSheetCurrent} aria-label="Include CURRENT in Multi-Version Sheet" /> CURRENT</label><b>● {ko ? '현재' : 'CURRENT'}</b><span>{date(formula.updatedAt)}</span><small>{ko ? '편집 가능한 현재 Formula' : 'Current working formula'}</small></div><div className="tm-restore-action"><span>{sheetSelectionCount} / 5 selected</span><button type="button" className="tm-restore-button" disabled={sheetSelectionCount < 2 || sheetSelectionCount > 5 || !onViewAsSheet} onClick={handleViewAsSheet}>VIEW AS SHEET</button></div><button className="tm-save" type="button" onClick={() => setNoteOpen(true)}>{ko ? '+ 버전 저장' : '+ SAVE VERSION'}</button>{noteOpen && <div className="tm-save-box"><label>{ko ? '버전 메모' : 'Version note'}<textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder={ko ? '실험 또는 시향 메모 (선택)' : 'Optional experiment or sensory note'} /></label><div><button className="btn" type="button" onClick={() => setNoteOpen(false)}>{ko ? '취소' : 'Cancel'}</button><button className="btn primary" type="button" disabled={saving} onClick={() => void save()}>{saving ? '…' : (ko ? '버전 저장' : 'Save Version')}</button></div></div>}{versions.length === 0 && <div className="tm-empty"><strong>{ko ? '저장된 버전이 없습니다.' : 'No saved versions yet.'}</strong><span>{ko ? '나중에 돌아올 수 있도록 의미 있는 Formula 단계를 저장하세요.' : 'Save a meaningful stage of your formula to return to it later.'}</span></div>}{[...versions].reverse().map((version) => <div key={version.versionId}><input type="checkbox" checked={sheetSelected.includes(version.versionId)} disabled={!sheetSelected.includes(version.versionId) && sheetSelectionCount >= 5} onClick={event => event.stopPropagation()} onChange={() => toggleSheetVersion(version.versionId)} aria-label={`Select ${version.kind === 'manual' ? `Version ${version.versionNumber}` : `restore point ${version.createdAt}`} for Multi-Version Sheet`} /><button className={`tm-item ${version.kind}`} type="button" onClick={() => { setSelected(version); setCompareTarget(undefined); setActiveTab("version") }}><span className="tm-version-item-heading"><b>{version.kind === 'manual' ? `○ v${version.versionNumber}` : `◇ ${ko ? '복원 지점' : 'RESTORE POINT'}`}</b>{releasedVersion?.versionId === version.versionId && <ReleasedIndicator language={language} />}</span><span>{date(version.createdAt)}</span>{version.note && <small>{formatVersionNote(version, language)}</small>}</button></div>)}</div>}
        </div>
