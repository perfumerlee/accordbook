...
export default function TimeMachinePanel({ formula, storage, language, onClose, onBeforeSaveVersion, onRestore, onCreateAsNew, onMarkReleasedVersion, onRemoveReleasedVersion, isOpen, opener, openSequence = 0 }: { formula: Formula; storage: AccordbookStorage; language: 'en' | 'ko'; onClose: () => void; onBeforeSaveVersion?: () => Promise<void>; onRestore?: (formula: Formula) => Promise<void>; onCreateAsNew?: (version: FormulaVersion) => Promise<void>; onMarkReleasedVersion?: (formulaId: string, versionId: string) => Promise<void>; onRemoveReleasedVersion?: (formulaId: string) => Promise<void>; isOpen: boolean; opener?: HTMLElement | null; openSequence?: number }) {
