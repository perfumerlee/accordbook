...
function GuideBookIcon() {
  return <svg className="first-start-guide-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
    <path d="M4.5 5.25c2.65 0 4.7.62 7.5 2.1v11.4c-2.8-1.48-4.85-2.1-7.5-2.1V5.25Z" fill="none" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M19.5 5.25c-2.65 0-4.7.62-7.5 2.1v11.4c2.8-1.48 4.85-2.1 7.5-2.1V5.25Z" fill="none" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
}
    ...
    {view === 'welcome' ? <>
      ...
      <div className="first-start-options">
        <button ref={createRef} type="button" className="first-start-option first-start-primary" onClick={onCreate}>
          ...
          <span>{ko ? '빈 포뮬러에서 시작합니다.' : 'Start from a blank formula.'}</span>
        </button>

        <label className="first-start-option" htmlFor="first-start-file">
          <strong>{ko ? '.accordbook 파일 열기' : 'Open .accordbook file'}</strong>
          <span>{ko ? '받거나 다운로드한 포뮬러를 엽니다.' : 'Open a formula you received or downloaded.'}</span>
          <input id="first-start-file" className="first-start-file" type="file" accept=".accordbook" onChange={(event) => { const file = event.target.files?.[0]; if (file) onImport(file); event.currentTarget.value = '' }} />
        </label>
      </div>

      <button type="button" className="first-start-starter-link" onClick={() => setView('starters')}>
        {ko ? '스타터 포뮬러 둘러보기 →' : 'Try a starter formula →'}
      </button>

      <a className="first-start-guide-link" href={`/guide/${language}/`}>
        <span className="first-start-guide-main">
          <GuideBookIcon />
          <span>
            <strong>{ko ? '가이드 보기' : 'Read the Guide'}</strong>
            <small>{ko ? '포뮬러 작성부터 버전 관리, 실험, 백업까지 알아보세요.' : 'Learn formulas, versions, experiments, and backup basics.'}</small>
          </span>
        </span>
        <span className="first-start-guide-arrow" aria-hidden="true">→</span>
      </a>
    </> : <>
      <p className="first-start-kicker">Accordbook</p>
      <h2 id="first-start-title">{ko ? '스타터 포뮬러' : 'Starter formulas'}</h2>
      <p className="first-start-subtitle">{ko ? '간단한 포뮬러로 Accordbook을 살펴보세요.' : 'Explore a simple formula and see how Accordbook works.'}</p>
      <div className="first-start-list">{starters.map((starter, index) => <button key={starter.id} type="button" className="first-start-starter" onClick={() => onStarter(starter)}><span className="first-start-number">{String(index + 1).padStart(2, '0')}</span><span><strong>{starter.title}</strong><small>{ko ? index === 0 ? '1,000파트 기본 흐름을 배워보세요.' : index === 1 ? '간단한 플로럴 구조를 살펴보세요.' : '우디 머스크 베이스를 살펴보세요.' : starter.description}</small></span><b>Open</b></button>)}</div>
      <button type="button" className="first-start-back" onClick={() => setView('welcome')}>← {ko ? '뒤로' : 'Back'}</button>
    </>}
