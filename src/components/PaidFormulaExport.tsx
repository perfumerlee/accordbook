...
function ExportDialog({ formula, language, storage, close }: { formula: Formula; language: 'en' | 'ko'; storage: AccordbookStorage; close: () => void }) {
  ...
  const sellerTokenRef = useRef<HTMLInputElement>(null)
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => { const target = sellerToken ? buyerNameRef.current : sellerTokenRef.current; if (target) target.focus({ preventScroll: true }); else closeRef.current?.focus({ preventScroll: true }) })
    return () => {
  ...
  return createPortal(<dialog ref={dialog} className="paid-export-dialog" aria-labelledby="paid-export-title" onCancel={event => { event.preventDefault(); if (!running.current) close() }}>
    <form autoComplete="off" onSubmit={async event => {
      ...
      <div className="paid-export-heading"><h2 id="paid-export-title">{ko ? '라이선스 Formula 내보내기' : 'Licensed Formula Export'}</h2><button ref={closeRef} className="paid-export-close" type="button" aria-label={ko ? '닫기' : 'Close'} onClick={close} disabled={busy}>×</button></div>
      ...
      <fieldset disabled={busy || registered}>
        <label>{ko ? '판매자 등록 키' : 'Seller registration token'}<input ref={sellerTokenRef} name="sellerToken" type="password" minLength={32} autoComplete="off" value={sellerToken} onChange={event => setSellerToken(event.target.value)} required /></label>
      </fieldset>
      ...
      <fieldset disabled={busy || !!issuance.current}>
        <label>{ko ? '구매자 이름' : 'Buyer name'}<input ref={buyerNameRef} name="buyerName" required maxLength={100} /></label>
