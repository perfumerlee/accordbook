...
export default function AccordbookNotebook() {
  ...
  const save = (next: Formula) => { setActive(next); setFormulas((all) => all.map((f) => f.id === next.id ? next : f)); schedule(next) }
  const update = (changes: Partial<Formula>) => { if (active) save({ ...active, ...changes, updatedAt: new Date().toISOString() }) }
  const createNew = async () => { if (!storage) return; flush(); const f = await createFormula(storage, prefix); await storage.formulas.save(f); setFormulas((all) => [...all, f]); setActive(f); localStorage.setItem(ACTIVE_KEY, f.id) }
  const duplicate = async () => { if (!storage || !active) return; flush(); const f = await duplicateFormula(storage, active, prefix); setFormulas((all) => [...all, f]); setActive(f); localStorage.setItem(ACTIVE_KEY, f.id) }
  ...
  const exportJson = async () => { if (!storage) return; flush(); downloadBackup(await createBackup(storage)) }
...
function Row({ row, onChange, onRemove, t }: { row: FormulaMaterial; onChange: (changes: Partial<FormulaMaterial>) => void; onRemove: () => void; t: { strength: string; solventCarrier: string; total: string; solvent: string; removeDilution: string } }) {
  ...
  const setDilution = (changes: Partial<NonNullable<FormulaMaterial['dilution']>>) => onChange({ dilution: { enabled: true, percent: 10, solvent: 'ALC', ...dilution, ...changes } })
  return <>
    <div className={`row ${row.marked ? 'marked' : ''}`}><input className="parts" type="number" step="1" min="0" value={row.parts} onChange={(e) => onChange({ parts: e.target.value === '' ? '' : Math.max(0, Math.trunc(Number(e.target.value))) })} /><div className="material-wrap"><input className="material" value={row.material} onChange={(e) => onChange({ material: e.target.value })} /><div className="material-meta">{notation && <span className="dilution-suffix">{notation}</span>}{isSolventMaterial(row.material) && <span className="solvent-badge">SOLVENT</span>}</div></div><input className="cas" value={row.cas ?? ''} onChange={(e) => onChange({ cas: e.target.value })} /><div className="percent">{calculatePercent(row.parts).toFixed(1)}%</div><div className="row-actions"><button className={`dilution-btn ${dilution ? 'active' : ''}`} type="button" onClick={() => { if (!dilution) setDilution({}); setOpen((value) => !value) }}>DIL</button><button className={`mark-btn ${row.marked ? 'active' : ''}`} type="button" onClick={() => onChange({ marked: !row.marked })}>▰</button><button className="remove-btn" type="button" onClick={onRemove}>×</button></div></div>
