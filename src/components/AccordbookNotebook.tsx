...
export default function AccordbookNotebook() {
  ...
  useEffect(() => { const onVisibility = () => { if (document.visibilityState !== 'visible') flush() }; document.addEventListener('visibilitychange', onVisibility); return () => { document.removeEventListener('visibilitychange', onVisibility); flush(); if (debounceTimer.current) clearTimeout(debounceTimer.current); if (provenanceTimer.current) clearTimeout(provenanceTimer.current); if (statusTimer.current) clearTimeout(statusTimer.current) } }, [storage])
