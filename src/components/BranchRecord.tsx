...
export default function BranchRecord({ variant, parentLabel, language }: { variant: ExperimentVariant; parentLabel?: string; language: 'en' | 'ko' }) {
  ...
  return <section className="branch-record" aria-label={ko ? '브랜치 기록' : 'Branch record'}>
    <header className="branch-record-header"><span>{ko ? '브랜치 기록' : 'BRANCH RECORD'} · {variant.label}</span><span className="branch-record-purpose"><span className="branch-record-purpose-label">{ko ? '생성 목적' : 'PURPOSE'}</span><strong>{purposeLabel}</strong></span></header>
    <div className={`branch-record-columns${origin && intent ? '' : ' branch-record-columns--single'}`}>
      {origin && <section className="branch-record-origin">
        <h3>{ko ? '이 브랜치가 만들어진 이유' : 'Why this Branch was created'}</h3>
      ...
      {intent && <section className="branch-record-intent">
        <h3>{ko ? '이번 브랜치에서 검증할 것' : 'What this Branch will test'}</h3>
