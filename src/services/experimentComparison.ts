...
export const EXPERIMENT_COMPARE_LIMIT = 5
const norm = (v?: string) => (v ?? '').trim().toLowerCase().replace(/\s+/g, ' ')
const dil = (v?: FormulaDilution) => v?.enabled ? `${v.percent}:${norm(v.solvent)}` : 'none'
const fallback = (a: FormulaSnapshotRow, b: FormulaSnapshotRow) => norm(a.cas) && norm(a.cas) === norm(b.cas) && norm(a.material) === norm(b.material) && dil(a.dilution) === dil(b.dilution)
...
function match(row: FormulaSnapshotRow, representatives: FormulaSnapshotRow[], used: Set<number>, stateRows: FormulaSnapshotRow[]) {
  const exact = row.rowId.trim() ? representatives.findIndex((candidate, i) => !used.has(i) && candidate.rowId === row.rowId && sameMaterial(row, candidate)) : -1
...
export function getDefaultComparisonVariantIds(experiment: Experiment): string[] { return experiment.variants.slice(0, EXPERIMENT_COMPARE_LIMIT).map((v) => v.variantId) }
export function getOrderedComparisonVariantIds(experiment: Experiment, variantIds: string[]): string[] { const selected = new Set(variantIds); return experiment.variants.filter((v) => selected.has(v.variantId)).slice(0, EXPERIMENT_COMPARE_LIMIT).map((v) => v.variantId) }
