...
import type { GuideKoTranslationReadiness } from './guideTranslation'
...
const koTranslationGuardGuideIds = new Set([
  'formula-basics',
  'experiments',
  'import-export',
  'data-backup',
...
export function requiresGuideKoTranslationGuard(guideId: string) {
  return koTranslationGuardGuideIds.has(guideId)
}

export function publisherGuidePath(guideId: string) {
  return publisherGuidePaths[guideId as keyof typeof publisherGuidePaths]
}
...
export function collectGuideAssetReferences(document: GuideDocument): string[] {
  ...
  for (const block of document.blocks) {
    ...
    for (const locale of Object.values(variants ?? {})) {
      for (const variant of Object.values(locale ?? {})) {
        if (variant?.src) paths.add(variant.src)
      }
    }
  }
  return [...paths].sort()
...
export function isSafeGuideAssetReference(src: string) {
  return /^assets\/[A-Za-z0-9._/-]+$/.test(src) && !src.includes('..') && !src.startsWith('/') && !/^[A-Za-z]:/.test(src)
}

export function serializePublishedGuide(document: GuideDocument) {
  return JSON.stringify(document, null, 2) + '\n'
}

export function publishTreeEntry(guideId: string, blobSha: string) {
  const path = publisherGuidePath(guideId)
  if (!path) throw new Error('UNKNOWN_GUIDE')
  return { path, mode: '100644' as const, type: 'blob' as const, sha: blobSha }
}

export function publishCommitPlan(guideId: string, parentSha: string, treeSha: string) {
  return { message: `content: publish guide ${guideId}`, tree: treeSha, parents: [parentSha], force: false as const }
}

export type PublishEligibility = { enabled: boolean; reason?: string }

export function confirmPublishIntent(
  title: string,
  guideId: string,
  revision: number,
  enStatus: string,
...
export function getPublishEligibility(input: {
  ...
  translationCheck?: GuideKoTranslationReadiness
}): PublishEligibility {
  ...
  if (!validation.ok) return { enabled: false, reason: 'Resolve publication validation issues before publishing.' }

  if (requiresGuideKoTranslationGuard(input.document.guideId)) {
    if (!input.translationCheck) return { enabled: false, reason: 'Checking Korean translation readiness…' }
    if (!input.translationCheck.ready) {
      return {
        enabled: false,
        reason: input.translationCheck.issues[0]?.message ?? 'Korean translation must be READY before publishing.',
      }
    }
  }
...
export type GuidePublisher = {
  publish(guideId: string, expectedDraftRevision: number): Promise<GuideDraftResult>
}
...
export function publishErrorMessage(result: Extract<GuideDraftResult, { ok: false }>) {
  ...
  const messages: Record<string, string> = {
    ...
    TRANSLATION_NOT_READY: 'Korean translation is not READY or its review fingerprints no longer match.',
...
export const guidePublisher: GuidePublisher = {
  async publish(guideId, expectedDraftRevision) {
    ...
    if (result.ok || result.code === 'PUBLISH_SUCCEEDED_DRAFT_REBASE_FAILED') {
      window.dispatchEvent(new Event('guide-publication-complete'))
    }
    return result
  },
}
