export type AccordbookRoute = { kind: 'root' } | { kind: 'drop-index' } | { kind: 'drop-detail'; dropId: string; slug: string } | { kind: 'formula-storage' } | { kind: 'unknown' }
export function resolveAccordbookRoute(pathname: string): AccordbookRoute {
  ...
  if (pathname === '/operator/formula-storage' || pathname === '/operator/formula-storage/') return { kind: 'formula-storage' }
