...
function aggregateDrop_(dropId, events, resetAt) {
  ...
  const rows = events.filter(event => event.dropId === dropId && (!resetTime || event.timestamp >= resetTime) && allowed.indexOf(event.eventType) >= 0);
